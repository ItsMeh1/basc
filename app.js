(() => {
  ...
  function render(route = routeFor()) {
    ...
    if (nav) nav.innerHTML = routes.filter(r => r.nav).map(r => `<a href="${esc(r.path)}" data-route="${esc(r.path)}" class="${normalize(r.path) === normalize(route.path) ? 'active' : ''}">${esc(r.label)}</a>`).join('');
    if (footerLinks) footerLinks.innerHTML = routes.filter(r => r.nav).map(r => `<a href="${esc(r.path)}" data-route="${esc(r.path)}">${esc(r.label)}</a>`).join('');
  ...
  function navigate(path, replace = false) {
    ...
    history[replace ? 'replaceState' : 'pushState']({ route: route?.path || target }, '', route ? route.path : target);
    render(route);
  ...
  function bindRoutes() {
    document.querySelectorAll('[data-route]').forEach(link => {
      link.addEventListener('click', event => {
        ...
        const route = routeFor(link.getAttribute('data-route'));
        if (!route) return;
